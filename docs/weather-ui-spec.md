...
`{available: true, calc_ts, age_s, stale, issued_values, zambretti: {text, confidence, letter, targets: [{target_ts, horizon_h}] × 2 (+6 ч/+12 ч)}, sager: {target_ts, text, confidence} | null, persistence: [{target_ts, horizon_h, t_out_c, p_rel_mmhg, rh_out_pct, wind_ms, confidence}] × 3 (+1 ч/+3 ч/+6 ч)}` (v1.2.6 — фактический конверт реализации U5, v0.4.0; дельта ревью владельца — `issued_values`, `letter`). Источник — последний прогон таблицы `forecast` (`MAX(issued_at)`), пишет `weather_aggregator` ежечасно (timer `*:02:00`); формулы Замбретти/Сагера в UI-сервере не дублируются. `stale: true`, если возраст расчёта > 2 ч (2 пропущенных прогона). **`issued_values`** — база Δ, семантика «от базы расчёта» (решение владельца — вариант (б)): `{t_out_c, p_rel_mmhg}` — последний замер `weather` с `ts <= issued_at` (НЕ текущие показания); `null` — истории до `issued_at` нет → клиент показывает Δ «—». **`zambretti.letter`** — буква Замбретти; пишет этап B в колонку `forecast.letter` (миграция без записи версии в `schema_migrations` — практика смоук-prepare для `text`: чек meta migrations ждёт версию копии БД 2); `null` — строки до миграции (легаси; клиент обязан переживать). Горизонт 24 ч этапом B не пишется — отклонён владельцем.
**Прогноза ещё нет** (пустая таблица `forecast`): **200** (не 503 — это не ошибка) с `{"available": false, "reason": ...}`. UI — карточка.
